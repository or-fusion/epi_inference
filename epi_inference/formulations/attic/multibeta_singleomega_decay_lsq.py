...
def run_multibeta_singleomega_decay_lsq(cm_rep_cases, populations, sigma, gamma, report_delay, reporting_factor, analysis_window, verbose=False):
    """
    ...
    m = create_inference_formulation_multibeta_singleomega_decay_lsq(
        Cdates=Cdates,
    ...
    for j in m.TIMES:
        ...
        if not m.omega[j].stale:
            est_omega[ dates[j] ] = value(m.omega[j])

    return {'est_beta': est_beta, 'est_omega':est_omega, 'status': 'ok', 'msg': 'Optimal solution found'}

def create_inference_formulation_multibeta_singleomega_decay_lsq(Cdates, cumulative_reported_cases, populations, sigma, gamma_1, gamma_2, gamma_3, report_delay, reporting_factor, analysis_window, loglsq=False, verbose=False):
    """
    ...
    model.omega = pe.Var(model.timesteps, initialize=1.3, bounds=(0,1)) # transmission parameter
    ...
    def block_rule(B, nodeid):
        ...
        def _lse(b):
            return sum( (b.T_hat[t] - T[t])**2 for t in TIMES)
